...
internal class ViewOpponent(
    private val model: Game
): VBox() , IView {


    private val label = Label("Opponent's Formation")
    ...
    init {
        ...
        for (i in 0..9) {
            for (j in 0..9) {
                ...
                if(model.getBoard(Player.AI)[j][i] == CellState.Ocean) {
                    rect.fill = Color.LIGHTBLUE
                } else if (model.getBoard(Player.AI)[j][i] == CellState.Attacked) {
                    rect.fill = Color.DARKBLUE
                } else if (model.getBoard(Player.AI)[j][i] == CellState.ShipHit) {
